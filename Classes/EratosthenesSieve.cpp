...
size_t EratosthenesSieve::nearestNext(size_t number, size_t next)
{
    if (number == 0) {
        return 0;
    }
...
int64_t EratosthenesSieve::linearFindGtIndexInList(size_t number, vector<size_t> bus)
{
    for (size_t i = 0; i < bus.size(); i++)
    {
        if (bus[i] > number) {
...
int64_t EratosthenesSieve::btsFindGtIndexInList(size_t number, vector<size_t> bus, int64_t leftMostIndex = -1, int64_t rightMostIndex = -1)
{
...
int64_t EratosthenesSieve::findGtIndexInList(size_t number, vector<size_t> bus, bool useLinear = false)
{
    if (useLinear) {
        // Slow O(n)
...
bool EratosthenesSieve::isNumberDivisibleByNumbersLt(size_t number, vector<size_t>& bus)
{
...
bool EratosthenesSieve::threadedIsFindResultDivisibleByLtNumbersInBuses(FindNextGtIndexResult result, vector<vector<size_t>> buses)
{
...
bool EratosthenesSieve::isFindResultDivisibleByLtNumbersInBuses(FindNextGtIndexResult result, vector<vector<size_t>> buses)
{
    ...
    for (size_t busIndex = 0; busIndex <= result.busIndex; busIndex++)
    {
        futures.push_back(async(isNumberDivisibleByNumbersLt, this, result.inBusFindResult.value, ref(buses[busIndex])));
    }
